...
def update_daily_counts(counts_collection, daily_chat_counts):
    ...
    counts_collection.update_one(
        {"_id": "chat_counts"},
        {"$set": {"chat_counts_per_day": daily_chat_counts}},
        upsert=True
    )

def mark_chats_as_counted(collection, chat_ids):
    """
    Update the processed chats by setting 'chat_counted' to True.
    """
    if chat_ids:
        collection.update_many(
            {"_id": {"$in": chat_ids}},
            {"$set": {"user_details.chat_counted": True}}
        )

def user_chat_counts():
    """
    ...
    if hour_counts or first_message_counts or daily_chat_counts:
        ...
        update_counts(counts_collection, hour_counts, "chat_counts")
        update_counts(counts_collection, first_message_counts, "chat_counts_first_message")
